      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6 justify-items-center">
          {foodItems.map((food, index) => (
            <div
              key={index}
              className="flex flex-col items-center justify-center  h-48 w-48 bg-white rounded-lg shadow-md transform transition-all duration-300 hover:scale-105 hover:shadow-lg"
            >
              {/* Adjusted image size */}
              <img
                src={food.image}
                ...
                loading="lazy"
              />
              <p className="text-gray-700 font-medium text-center">{food.name}</p>
            </div>
          ))}
        </div>
      </div>
